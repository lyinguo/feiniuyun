...
app_api = FastAPI(title="小说转剧本 AI 后端")
# 1. 基础配置加载
load_dotenv(r'../.env')
...
def run_pipeline(json_path: str, output_dir: str):
    ...
    for chapter in chapters:
        ...
        # 核心修改：后缀变为 .json
        script_file = out_path / f"chapter_{idx:03d}_script.json"
...
@app_api.post("/api/v1/run_conversion")
async def run_conversion_api(json_path: str, output_dir: str):
    """前端调用这个接口来启动工作流"""
    
    # 因为 LangGraph 目前是同步跑的，我们需要把它放到后台线程去跑，以免阻塞主 Web 线程
    import threading
    
    def background_task():
        # 这里把原本 run_pipeline 里的 for chapter in chapters 逻辑放进来
        # 在处理每一章前，可以推入队列： stream_queue.put_nowait({"type": "chapter_start", "chapter": title})
        try:
            run_pipeline(json_path, output_dir) # 调用你原来的主闭环函数
            stream_queue.put_nowait({"type": "pipeline_complete", "status": "success"})
